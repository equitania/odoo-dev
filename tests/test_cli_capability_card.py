"""Tests for the capability-card command."""
...
from odoodev.commands import capability_card as cc_module
...
class TestCapabilityCard:
    ...
    def test_missing_card_errors_cleanly(self, monkeypatch):
        monkeypatch.setattr(cc_module, "_find_card", lambda: None)
        runner = CliRunner()
        result = runner.invoke(cli, ["capability-card"])
...
class TestVersionInjection:
    def test_stale_version_is_replaced(self, monkeypatch, tmp_path):
        ...
        card.write_text("# X — Agent Capability Card\n- **Version:** 0.1.0  ·  **Framework:** Python / Click\n")
        monkeypatch.setattr(cc_module, "_find_card", lambda: card)
        runner = CliRunner()
        result = runner.invoke(cli, ["capability-card"])
        assert result.exit_code == 0
        assert f"**Version:** {__version__}" in result.output
