...
from odoodev.core.global_config import (
    ...
    DEFAULT_DB_PASSWORD,
    DEFAULT_DB_USER,
    DEFAULT_ODOO_LOGIN_PASSWORD,
    DEFAULT_ODOO_LOGIN_USERNAME,
    DatabaseConfig,
    GlobalConfig,
    OdooLoginConfig,
    clear_config_cache,
    config_exists,
    get_odoo_login_credentials,
    load_global_config,
    save_global_config,
    save_odoo_login_credentials,
)
...
class TestOdooLoginConfig:
    """Odoo XML-RPC login section (v0.59.0)."""

    def test_defaults(self):
        cfg = GlobalConfig()
        assert cfg.odoo_login.username == DEFAULT_ODOO_LOGIN_USERNAME == "admin"
        assert cfg.odoo_login.password == DEFAULT_ODOO_LOGIN_PASSWORD == "admin"

    def test_defaults_when_absent_from_yaml(self, config_dir):
        """Configs written before v0.59.0 have no odoo_login section."""
        config_dir.parent.mkdir(parents=True, exist_ok=True)
        config_dir.write_text("base_dir: ~/legacy\n")
        loaded = load_global_config()
        assert loaded.odoo_login.username == "admin"
        assert loaded.odoo_login.password == "admin"

    def test_roundtrip(self, config_dir):
        original = GlobalConfig(odoo_login=OdooLoginConfig(username="exporter", password="s3cret"))
        save_global_config(original)
        clear_config_cache()
        loaded = load_global_config()
        assert loaded.odoo_login.username == "exporter"
        assert loaded.odoo_login.password == "s3cret"

    def test_get_credentials_helper(self, config_dir):
        save_global_config(GlobalConfig(odoo_login=OdooLoginConfig(username="u1", password="p1")))
        assert get_odoo_login_credentials() == ("u1", "p1")

    def test_save_credentials_helper_preserves_other_fields(self, config_dir):
        save_global_config(GlobalConfig(base_dir="~/keep-me"))
        save_odoo_login_credentials("u2", "p2")
        clear_config_cache()
        loaded = load_global_config()
        assert loaded.odoo_login == OdooLoginConfig(username="u2", password="p2")
        assert loaded.base_dir == "~/keep-me"

    def test_saved_file_is_owner_only(self, config_dir):
        save_odoo_login_credentials("u3", "p3")
