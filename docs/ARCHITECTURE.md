# Architecture Reference — odoodev

Extracted from `CLAUDE.md` so it costs nothing until it is actually needed.
`CLAUDE.md` is loaded into every request; this file is read on demand.

**Read this when** you are changing a core module, adding a command, or need to know
what `start` checks before launching. For using the CLI (flags, recipes, contracts),
`usage/AGENT.md` is the better source — it tracks `--help` output.
...
| `requirements` | sync / diff / adopt / prune — baseline (wheel) + `requirements.local.txt` overlay → generated `requirements.txt` (v0.63.0). Since v0.64.0 `adopt --yes` takes the BASELINE for a conflicting pin (a hand-maintained file predates the baseline, so its pins are the old state, and keeping them reverted the security baseline wholesale — on v16 into an unsolvable set against `eq-chatbot-core`'s cryptography floor); `--keep-local` restores the old behaviour. A failed `uv pip install` now names `requirements.local.txt` and `requirements diff` as where to look. |
