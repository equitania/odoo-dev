...
`requirements.txt` einmalig verlustfrei in Baseline + Overlay (Backup als `.pre-adopt`) — und
verweigert sich, sobald `requirements.local.txt` bereits Einträge enthält (dann ist `sync` der
richtige Befehl, nicht ein erneutes `adopt`). Hält ein Overlay-Pin ein Baseline-Update zurück (etwa
`Werkzeug==3.0.6` unter v16, weil Odoos `http.py` das in Werkzeug 3.1 entfernte
`werkzeug.__version__` liest), meldet jeder Sync das explizit.
...
- **Sicherheit:** `requirements adopt` verweigert den Lauf jetzt, sobald `requirements.local.txt`
  bereits Einträge enthält, und verweist auf `sync`. Die alte Prüfung fragte nur den generierten
  Header der aktuellen `requirements.txt` ab — kein dauerhaftes Signal, denn ein `git pull` im
  geteilten `vXX-dev`-Repo oder ein Kollege mit älterem odoodev kann diesen Header entfernen, ohne
  das Overlay anzufassen. Ein erneutes `adopt` hätte das Overlay dann mit dem Inhalt der veralteten
  `requirements.txt` überschrieben und jede seitherige Handbearbeitung verloren. `adopt` sichert
  `requirements.local.txt` zusätzlich als `.pre-adopt`, genau wie schon `requirements.txt`.
...
into baseline + overlay once, losslessly (keeps a `.pre-adopt` backup) — and refuses once
`requirements.local.txt` already has entries (at that point `sync` is the right command, not
another `adopt`). When an overlay pin holds back a baseline update — e.g. `Werkzeug==3.0.6` on
v16, because Odoo's `http.py` reads `werkzeug.__version__`, which Werkzeug 3.1 removed — every
sync reports it explicitly.
...
- **Safety:** `requirements adopt` now refuses to run once `requirements.local.txt` already has
  entries, and points at `sync`. The earlier guard only checked the generated header on the
  *current* `requirements.txt` — not a durable signal, since a `git pull` in the shared `vXX-dev`
  repo, or a colleague on an older odoodev, can strip that header without ever touching the
  overlay. A second `adopt` would then have overwritten the overlay with only what that stale
  `requirements.txt` implied, discarding every hand edit made since. `adopt` also backs up
  `requirements.local.txt` to `.pre-adopt`, just like it already did for `requirements.txt`.
