...
- **`core/database.py`** — PostgreSQL ops via `psql`/`createdb`/`dropdb` CLI. Backup extraction (ZIP, 7z, tar, gz, SQL). Post-restore: deactivates cron jobs and cloud integrations, and (on by default) anonymizes personal data via `anonymize_database()` — Faker-generated, per-id-seeded values applied as bundled `UPDATE ... FROM (VALUES ...)` over `res_partner`/`res_users`/`crm_lead`/`res_partner_bank` plus whole-table wipes of `mail_message`/`ir_attachment`; emails/logins forced to RFC 2606 reserved targets. Default credentials: `ownerp`/`CHANGE_AT_FIRST`.
