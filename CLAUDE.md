...
- **`core/database.py`** — PostgreSQL ops via `psql`/`createdb`/`dropdb` CLI. Backup creation (`backup_database_sql`, `create_backup_zip`, `create_backup_tar_zst` — the last pipes a Python `tarfile` stream into the `zstd` CLI, symmetric to the `.tar.zst` restore). Backup extraction (ZIP, 7z, tar, tar.zst, gz, SQL). Post-restore (all on by default): (1) `deactivate_cronjobs()` psql baseline (crons/mail/fetchmail), (2) native `run_neutralize()` → `odoo-bin neutralize` (runs each module's `data/neutralize.sql`: payment/IAP/webhooks/banner/cloud modules; graceful-skip if venv/odoo-bin/conf missing), (3) `anonymize_database()` — Faker-generated, per-id-seeded values as bundled `UPDATE ... FROM (VALUES ...)` over `res_partner`/`res_users`/`crm_lead`/`res_partner_bank` + whole-table wipes of `mail_message`/`ir_attachment`; emails/logins forced to RFC 2606 reserved targets. Standalone `odoodev db neutralize`. Default credentials: `ownerp`/`CHANGE_AT_FIRST`.
