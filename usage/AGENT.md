...
| `odoodev requirements adopt` | One-time lossless migration of a hand-maintained `requirements.txt` into baseline + `requirements.local.txt` overlay (backs up the original as `requirements.txt.pre-adopt`). Required before `sync` will touch a hand-maintained file. | [VERSION], --yes/-y (keep every local pin without prompting) |
