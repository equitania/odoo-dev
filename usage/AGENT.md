...
- **Version:** 0.63.0  ·  **Framework:** Python / Click
...
- **Requirements baseline/overlay (v0.63.0):** `requirements.txt` is generated from a shipped
  baseline plus a machine-local `requirements.local.txt` overlay that odoodev never overwrites.
  `requirements sync`/`diff`/`adopt` manage it; `start` auto-regenerates it when the baseline moved.
...
| `odoodev requirements adopt` | One-time lossless migration of a hand-maintained `requirements.txt` into baseline + `requirements.local.txt` overlay (backs up the original as `requirements.txt.pre-adopt`, and the overlay itself as `requirements.local.txt.pre-adopt`). Required before `sync` will touch a hand-maintained file. Refuses once `requirements.local.txt` already has entries — run `sync` instead at that point. | [VERSION], --yes/-y (take the baseline for every conflict without prompting; entries the baseline does not know still move to the overlay), --keep-local (keep every local pin instead, overriding the baseline) |
...
- **`odoodev start` prerequisites** (checked before launch): `.env`, `.venv/`, `odoo-bin`, a dated
  ...
  `requirements.txt` SHA256 (offers an update if it changed). Since v0.63.0 the SHA256 check runs
  after an automatic regeneration: if the shipped baseline moved on since the last sync,
  `requirements.txt` is rebuilt from baseline + `requirements.local.txt` first, so the hash check
  always sees the current effective file. `odoodev docker up` waits for the same
  readiness before reporting success.
- **`requirements sync` guard:** refuses to overwrite an existing `requirements.txt` that was not
  generated by odoodev unless a `requirements.local.txt` overlay already exists (that existence
  counts as consent — the environment has been through `adopt`). Run `odoodev requirements adopt`
  first on a hand-maintained file; never bypass this by deleting the file to force a sync.
- **`requirements adopt` guard:** refuses to run once `requirements.local.txt` already has real
  entries — that content, not the generated header on `requirements.txt`, is the durable
  "already adopted" signal, since a `git pull` in a shared `vXX-dev` repo (or an older odoodev)
  can strip the header without ever touching the overlay. At that point `sync` is the correct
  command, not another `adopt`.
...
- `odoodev requirements diff --json` → `{"version": "18", "rows": [{"name", "base", "local",
  "installed", "status"}]}` per package; `status` is exactly one of `"local override"` (overlay
  replaces a baseline pin), `"local only"` (overlay entry with no baseline counterpart),
  `"not installed"` (baseline entry, nothing installed), `"ok"` (baseline entry, installed
